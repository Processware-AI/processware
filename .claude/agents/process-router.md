---
name: process-router
description: 자연어 입력을 받아 MAT-007 프로세스 카탈로그를 기반으로 가장 적합한 WI(또는 PRO)를 매칭한다. /process-do 커맨드의 start 모드에서 인자가 doc_id 형식이 아닐 때 위임된다. (차원 2 Do Phase 3)
...
본 에이전트는 **WI 를 실행하지 않는다**. 매칭 결과만 호출자(/process-do)에게 반환한다.
...
```yaml
query: "공급자 평가 시작"          # 사용자 자연어 입력 (한/영/혼합)
...
options:
  ...
  matching_strategy: "llm_direct" # llm_direct (Phase 3 기본) | rag_vector (Phase 3.5+, pgvector 미구축 시 fallback to llm_direct)
  use_history: true               # Phase 3.5 — .claude/runs/ 의 자주 쓰는 doc_id 가중치 (+0.05~0.1)
  language_hint: "auto"           # auto | ko | en (자동 감지가 기본)
```
...
A-1. `MAT-007_프로세스_카탈로그.md` Read.
   - 파일 미존재 → 에러 반환: "MAT-007 카탈로그가 없습니다. `/process-do --rebuild-catalog` 또는 차원 1 빌드를 먼저 실행하세요."
...
B-0. **matching_strategy 분기** (Phase 3.5 신규):
   - `llm_direct` (기본) → B-1 단일 LLM 추론.
   - `rag_vector` → 외부 pgvector 인덱스 호출 (인터페이스만 정의, 실제 통합은 별도 PR).
     - pgvector 미구축 환경에서는 자동으로 `llm_direct` fallback + trace 에 `rag_unavailable_fallback` 메모.
     - 호출 인터페이스 (Phase 3.5 hook):
       ```
       POST {RAG_ENDPOINT}/match
       { "query": "...", "top_k": 3, "scope_filter": ["CMMI"] }
       → [ {doc_id, score, metadata}, ... ]
       ```

B-0-bis. **language 감지 (Phase 3.5)**:
   - `language_hint == "auto"` 시 query 의 한·영 비율로 결정.
   - 영어 비율 ≥ 70% → 영어 매칭 활성. 카탈로그 aliases[] 의 영어 표현 우선 매칭 + 한글 카탈로그를 영어 의미로 cross 매핑.
   - 매핑 우회 (Phase 3.5 추가):
     - "supplier evaluation" → "공급자 평가" → SAM
     - "non-conformance" → "부적합" → PQA
     - "configuration audit" → "구성감사" → CM
     - "root cause" → "근본원인" → CAR
     - "alternative selection" → "대안 선택" → DAR
...
B-1. **단일 LLM 추론 호출** (본 에이전트 자체가 LLM) — 다음 사고 절차:
   ...
   - **다국어 cross 매핑**: B-0-bis 에서 감지한 영어 입력은 위 매핑표 + LLM 의미 추론 병행.
...
B-2-bis. **사용자 이력 가중치 (Phase 3.5)** — `options.use_history == true` 시:
   - `Glob ".claude/runs/run-*/state.yaml"` 전수 → `wi_id` 별 실행 빈도 카운트 집계.
   - 최근 30일 빈도가 높은 doc_id 의 confidence 에 보너스 가중:
     - top 3 doc_id: +0.10
     - top 4~10: +0.05
     - 그 외: 0
   - **단 자동 채택은 보너스 적용 전 score 기준으로 판단** (이력 학습이 잘못된 매칭을 자동 채택으로 끌어올리는 것 방지).
   - trace 에 `history_boost_applied` 이벤트 (어떤 doc_id 에 +얼마 가중).
...
## 6. Phase 3.5 동작 사항 (현 단계)

**해제된 제약**:
- ✅ `matching_strategy` 옵션 — llm_direct / rag_vector 분기 (rag_vector 는 인터페이스 hook 만, 실제 pgvector 미구축)
- ✅ 다국어 매칭 — 영어 입력 → 한글 카탈로그 cross 매핑 (5개 핵심 매핑 + LLM 의미 추론)
- ✅ 사용자 이력 학습 — `.claude/runs/` 의 wi_id 빈도 → top 3/10 가중치 +0.05~0.10

**남은 제약 (Phase 3.6+)**:
- pgvector 실제 구축 — 카탈로그 1000건 초과 시 우선순위 ↑
- 트리거 이벤트 자동 탐지 — Jira/Git PR/캘린더 → WI 자동 추천 (외부 MCP)
- 사용자별 개인화 — 현재는 전체 익명 빈도. 사용자 ID 별 분리는 별도 PR.
