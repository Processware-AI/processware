...
> revision_plan + root_cause + PCB 승인 → **As-Is 입력 파일 + MAT-001 행 + 큐 done + /process-plan 재트리거 명령 (stdout)**.

대화는 하지 않는다. 입력이 부족하면 호출자(/process-act)에게 즉시 에러 반환.
...
rebuild_command: "/process-plan CMMI-DEV-ML3 --from write --target PRO-CMMI-04-01"
...
> `/process-plan CMMI-DEV-ML3 --from write --target PRO-CMMI-04-01` 실행 시 process-designer / wi-tmp-writer 가 본 파일을 읽고 개정에 반영합니다.
...
2. ▶ **rebuild** — `/process-plan CMMI-DEV-ML3 --from write --target PRO-CMMI-04-01`
3. ▶ **validate** — qa-reviewer 자동 호출 (build-process 내부).
4. ✅ **register** (완료) — 본 파일 작성 시 MAT-001 §개정 이력 행 추가.
5. ▶ **close_ncr** — `/process-check --close-ncr REC-NCR-04-01-2026-001 --capa <개정판 PRO 의 후속 REC>`
6. ▶ **re_kpi** — `/process-check --kpi start CMMI-DEV-ML3 --period <다음 분기>`
...
> 직접 수정 금지 — 추가 피드백은 새 act 사이클 (`/process-act start <queue_id>`).
```
...
> 차원 4 (Act) `/process-act` 하네스가 PCB 승인 후 자동 갱신. 각 개정 사이클별 1행.
...
▶ 차원 1 재실행 후:
  ...
  /process-check --close-ncr REC-NCR-04-01-2026-001 --capa <개정판 PRO 의 후속 REC>
  /process-check --kpi start CMMI-DEV-ML3 --period 2026-04-01..2026-06-30  (회차 2 측정)
```
...
### 3.5 차원 1 미실행 보장
- 본 에이전트는 `/process-plan` 를 **절대 실행 안 함**. 명령만 stdout.
