...
description: HITL 정지된 trace 들의 타임아웃 검출·에스컬레이션 체인 라우팅을 전담. /do --check-timeouts 호출 시 진입. (차원 2 Do Phase 2.5)
...
### 1-1. `scan` 모드 (기본 — `/do --check-timeouts`)
```yaml
mode: scan
options:
...
- 외부 cron / scheduler 자동 호출 — 사용자가 `/do --check-timeouts` 수동 실행 또는 외부 cron 등록 (예: `*/30 * * * * /do --check-timeouts`).
