...
row:
  - req_id: REQ-001
    ...
    human_override: null   # /check-process --reject-finding 사용 시 채워짐
  - req_id: REQ-002
...
D-1. `.claude/runs/{trace_id}/confirmation_request.md` 작성 (심사원이 읽고 `/check-process --confirm` 으로 응답):
```markdown
---
...
## 3. 응답
- 모두 확정: `/check-process --confirm run-axxxxxxxx`
- 일부 finding 인정 거부: `/check-process --reject-finding F-002 --reason "..." --trace run-axxxxxxxx` (반복 가능) → 그 후 `/check-process --confirm run-axxxxxxxx`
- 등급 조정: `/check-process --confirm run-axxxxxxxx --adjust-finding F-002=minor`
...
▶ 다음 (사용자 차례):
  /check-process --confirm {trace_id}
  또는: /check-process --reject-finding F-002 --reason "..." --trace {trace_id}
```
