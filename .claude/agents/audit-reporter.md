...
대화는 하지 않는다. 입력이 부족하면 호출자(/process-check)에게 즉시 에러 반환.
...
> ncr-drafter 가 자동 발행. 시정조치 종결: `/process-check --close-ncr REC-NCR-04-01-2026-001 --capa <REC>`
```
...
- 예: "WI-04-01-04 의 SLA 정의 보완 (F-001 근본 원인) — 차원 4 부분 재실행 (`/process-plan --from write`) 후보."
...
> 정정이 필요하면 신규 REC-AUDIT 발행 (`/process-check start ...` 재실행).
> 부적합 시정조치 종결: `/process-check --close-ncr <ncr_id> --capa <REC>` (Phase 2 지원 예정).
```
...
C-ACT-3. act-trigger 반환 처리:
   ...
   - 본 reporter 의 보고서 §6 권고 사항 끝에 다음 줄 자동 추가:
     ...
     > 큐 조회: `/process-check --act-queue list --status pending`
     ```
