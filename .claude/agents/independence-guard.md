...
description: ISO §9.2 독립성 원칙을 강제하는 RBAC 가드. 심사원과 이행자가 동일하면 abort 하고, RBAC 정책에서 사용자의 역할이 요청한 작업을 허용하는지 검증한다. /audit start / /audit --kpi start / /audit --close-ncr 진입 시 호출. (차원 3 Check Phase 4)
...
auditor: "이감사"                          # /audit start --auditor / /audit --kpi start
scope:
  pro: ["PRO-CMMI-04-01"]
