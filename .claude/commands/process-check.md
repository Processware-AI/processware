---
description: 표준 프로세스 심사 (차원 3 Check) — REC ↔ PRO/WI 요건 자동 대조 + 부적합 탐지 + 심사 보고서 발행. ISO §9.2 독립성 강제. 사용 ./process-check start <PRO|WI|범위> --auditor "이름" | --confirm <trace> | --status <trace>
...
본 커맨드는 차원 2 (`/process-do`) 가 누적한 **REC + trace.jsonl + MAT-005** 를 입력으로, AI 심사원이 **PRO/WI 요건 ↔ REC 충족 여부**를 자동 대조하여 **심사 보고서·부적합·KPI** 를 산출한다.
...
- **독립성 강제 (ISO 9.2)**: `/process-check start` 진입 시 `--auditor` 이름과 대상 trace 들의 `executed_by` 가 1건이라도 일치하면 즉시 abort. PoC 한정 `--override-independence` 만 허용 (Phase 1 만, Phase 4 에서 제거).
...
### 1-1. `start` 모드 — 신규 심사
```
/process-check start PRO-CMMI-04-01 --auditor "이감사"            # 단일 PRO 범위
/process-check start WI-CMMI-04-01-03 --auditor "이감사"           # 단일 WI 범위 (좁은 심사)
/process-check start CMMI-DEV-ML3 --auditor "이감사"               # 표준 전체 범위 (넓은 심사)
/process-check start PRO-CMMI-04-01 --auditor "이감사" --period 2026-01-01..2026-04-30
```
→ 인자 파싱:
...
### 1-2. `resume` 모드 — `/process-check --resume <trace_id>`
```
/process-check --resume run-a1c2d3e4
```
...
### 1-3. `confirm` 모드 — `/process-check --confirm <trace_id> [--auditor <이름>] [--adjust-finding <id>=<grade>]`
```
...
### 1-4. `reject-finding` 모드 — `/process-check --reject-finding <finding_id> --reason "..." [--trace <trace_id>]`
```
/process-check --reject-finding F-002 --reason "AI 오탐 — REC §3 에 시정 권고 이미 기재" --trace run-a1c2d3e4
```
→ AI 가 부적합으로 판정한 항목을 심사원이 인정하지 않음 (오탐 제거). 매트릭스에서 해당 row 의 `human_override: rejected` 로 마크. `--reason` 필수.

### 1-5. `status` 모드 — `/process-check --status <trace_id>`
```
/process-check --status run-a1c2d3e4
```
→ trace 의 진행 단계·발견 부적합 수·소요 시간 출력.

### 1-6. `list-ncr` 모드 — `/process-check --list-ncr [--status open|closed|all] [--standard <코드>] [--severity critical|major|minor] [--overdue]`
```
/process-check --list-ncr                                      # open 인 NCR 모두 (기본)
/process-check --list-ncr --status all                         # open + closed 모두
/process-check --list-ncr --status closed --standard CMMI-DEV-ML3
/process-check --list-ncr --severity critical --overdue        # SLA 기한 경과한 critical NCR
```
...
### 1-7-K. `kpi` 모드 (Phase 3) — `/process-check --kpi <subcommand> [...]`

KPI 측정·시계열 누적·회귀 탐지를 담당하는 단일 진입점. 4 subcommand.

```
/process-check --kpi start CMMI-DEV-ML3 --period 2026-01-01..2026-04-30
/process-check --kpi start PRO-CMMI-04-01 --period 2026-01-01..2026-04-30 --baseline auto
/process-check --kpi show CMMI-DEV-ML3                                   # MAT-008 표준 섹션 stdout 출력
/process-check --kpi show CMMI-DEV-ML3 --round 2                         # 특정 회차만
/process-check --kpi update CMMI-DEV-ML3                                 # 별칭 — start 와 동일 (period 미지정 시 자동 분기)
/process-check --kpi check-regressions                                   # 모든 표준의 critical+watch 알림 출력 + --overdue 가능
```
...
### 1-7-AQ. `act-queue` 모드 (Phase 4) — `/process-check --act-queue <subcommand> [...]`

차원 4 인계 큐를 조회·관리하는 단일 진입점. 4 subcommand.

```
/process-check --act-queue list                                          # 모든 큐 (status: pending 이 default)
/process-check --act-queue list --status all                             # 전체
/process-check --act-queue list --priority critical --kind ncr_capa
/process-check --act-queue show queue-q3a8f2c1                           # 단일 큐 상세
/process-check --act-queue dispatch queue-q3a8f2c1 --to "박팀장"          # process_owner 등에게 dispatch
/process-check --act-queue done queue-q3a8f2c1 --capa REC-CMMI-...       # 차원 4 완료 후 종결
```

**4 subcommand 동작**:
- `list`: `Glob ".claude/queues/process-act/queue-q*.yaml"` 전수 스캔 + 필터 적용 후 표 stdout.
...
- `done <queue_id> --capa <REC>`: 큐 status: in_progress → done, done_at / done_capa_rec 채움. 본 큐가 NCR 연계면 NCR 도 자동 close 권장 (별도 `/process-check --close-ncr` 호출 안내).

### 1-7-RC. `rbac-check` 모드 (Phase 4) — `/process-check --rbac-check --action <id> [--target <id>]`

```
/process-check --rbac-check --action audit.start --target PRO-CMMI-04-01
/process-check --rbac-check --action audit.close-ncr --target REC-NCR-04-01-2026-001
```
...
### 1-7. `close-ncr` 모드 — `/process-check --close-ncr <ncr_id> --capa <REC> [--closed-by <이름>] [--reason "..."]`
```
/process-check --close-ncr REC-NCR-04-01-2026-001 --capa REC-CMMI-04-01-04-01-2026-003
/process-check --close-ncr REC-NCR-04-01-2026-001 --capa REC-CMMI-04-01-04-01-2026-003 --closed-by "박팀장" --reason "SLA 정의 + 재실행 정상 승인"
```
...
D-2. 매트릭스 작성 완료 → state.status: `pending_confirmation` + `confirmation_request.md` drop-out (심사원에게 매트릭스 미리보기 + `/process-check --confirm` 안내).
...
- (no_act_queue=false 일 때, Phase 4) act-trigger 자동 위임 → .claude/queues/process-act/queue-q*.yaml N건 + MAT-008 §"차원 4 인계" 갱신
...
KA-6. **act-trigger 자동 위임 (Phase 4, options.no_act_queue == false 일 때)**:
   ...
   - .claude/queues/process-act/queue-q*.yaml N건 발행 + MAT-008 §"차원 4 인계" 표 갱신.
KA-7. 종결 보고.
...
| 3 | **kpi-collector** / **kpi-analyzer** / **MAT-008** KPI 대시보드 / `/process-check --kpi start\|show\|update\|check-regressions` / PRO/WI §KPI 표 자동 추출 / 메타 KPI 5종 (Coverage / Findings density / Independence / NCR 종결율 / NCR SLA) / 4-tier verdict (healthy/watch/recovering/critical/data_gap) / baseline seed + 회귀 임계 (default ±5%p) / MAT-009 §통계 자동 갱신 hook | RBAC 정식 / 외부 시스템 / 차원 4 자동 트리거 / 시계열 시각화 |
| **4 (지금)** | **independence-guard** 정식 분리 (independence_check + rbac_check 2 모드) / **RBAC** policy.yaml 6 역할 / **act-trigger** 에이전트 / 차원 4 인계 큐 (.claude/queues/process-act/queue-q*.yaml) / `--act-queue list\|show\|dispatch\|done` / `--rbac-check` / `--no-act-queue` 옵션 / MAT-008 §"차원 4 인계" 자동 누적 / Mermaid 시계열 시각화 (PoC) / 영업일 계산기 명세 (가이드 부록 B, 휴리스틱 28일 유지) | 외부 인증기관 보고서 (XLSX/PDF) → Phase 4.5 / 외부 IdP 연동 → Phase 4.5 / 큐 dispatch 외부 시스템 알림 → Phase 4.5 |
...
- 쓰기 허용:
  ...
  - `.claude/queues/process-act/queue-q*.yaml` (Phase 4 — 신규 / dispatch / done 시 Edit)
...
▶ 다음: /process-check --confirm run-a1c2d3e4
   또는 부적합 인정 거부: /process-check --reject-finding F-002 --reason "..." --trace run-a1c2d3e4
```
...
▶ 다음 (시정조치 종결):
  /process-check --close-ncr REC-NCR-04-01-2026-001 --capa <시정조치 REC>
  /process-check --list-ncr --overdue            # SLA 경과 NCR 점검
```
...
▶ 다음:
  /process-check --kpi show CMMI-DEV-ML3              # 결과 확인
  /process-check --kpi check-regressions              # 전사 알림 모음
  /process-check --kpi start CMMI-DEV-ML3 --period 2026-04-01..2026-06-30   # 2회차 (baseline 비교 자동)
```
