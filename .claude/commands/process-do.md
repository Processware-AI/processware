...
본 커맨드는 `/process-plan` 가 만들어 놓은 자산(WI/TMP/EX/PRO/POL)을 **계약서**로 삼아, AI Agent 가 사람과 대화하며 절차를 이행하고 REC(기록본)를 자동 작성한다.
...
- **HITL 강제 정지·재개**: WI §5.3 "완료 조건" 에 "승인" 표현이 있거나, WI §2 "수행 주체" 의 "승인자" 역할이 있으면 반드시 사람 승인 게이트 발동. Phase 2 부터 진짜 정지 (`pending_approval`) + 외부 채널 drop-out + `/process-do --approve|--reject` 응답 후 재개.
...
### 1-1. `start` 모드 — 신규 실행
```
/process-do WI-CMMI-04-01-03                 # 직접 doc_id
/process-do WI-CMMI-04-01-03_작업산출물_평가_v1.0    # 직접 파일명
/process-do 작업산출물 평가                   # 자연어 (Phase 3) — process-router 위임
/process-do 공급자 평가                       # 자연어 + 후보 제시 모드 가능
```
→ 인자 파싱:
...
### 1-2. `resume` 모드 — `/process-do --resume <trace_id>`
```
...
### 1-3. `approve` 모드 — `/process-do --approve <trace_id> [--approver <이름>]`
```
...
### 1-4. `reject` 모드 — `/process-do --reject <trace_id> --reason "..." [--approver <이름>]`
```
...
### 1-7-bis. `check-timeouts` 모드 — `/process-do --check-timeouts [--dry-run]`
```
/process-do --check-timeouts                # 모든 pending_approval trace 의 타임아웃 검출 + 에스컬레이션
...
권장 운영: 외부 cron 으로 30분 또는 1시간 주기 실행 (예: `*/30 * * * * /process-do --check-timeouts`).

### 1-7. `rebuild-catalog` 모드 — `/process-do --rebuild-catalog [--scope <영역>]`
```
/process-do --rebuild-catalog                # 전체 표준 재인덱싱
/process-do --rebuild-catalog --scope CMMI   # CMMI 만
```
...
B-3. **HITL 게이트 만나면 `state.status: pending_approval` 로 정지 → 종료** (사용자가 응답할 차례).
   ...
   - 사용자에게 응답 명령 (`/process-do --approve` / `/process-do --reject`) 안내.
...
- input_source: "/process-do --approve" or "/process-do --reject"
```
...
| 2 | HITL 정지(`pending_approval`) / 외부 채널 drop-out / `/process-do --resume\|--approve\|--reject\|--status\|--check-approvals` / 반려 처리 (REC.status: rejected) / 6개 진입 모드 | 자연어 라우팅 / 타임아웃·에스컬레이션 / 외부 IdP 인증 / 다단계 승인 |
