...
related_tmp:
  - "[[TMP-SPICE-01-01-01-01_이해관계자요구사항명세서]]"
related_rec: []
...
# 이해관계자 요구사항 도출 업무지침 (WI-SPICE-01-01-01)

> 상위 절차: [[PRO-SPICE-01-01_시스템공학프로세스]] §5 단계 1~3
...
VWAY Motors 가 OEM 으로 인도하는 차량 ECU·도메인 컨트롤러·ADAS 시스템의 신규 개발 및 중대 변경 프로젝트에 적용한다. RFQ 수령일부터 StRS v1.0 베이스라인 등록일까지의 활동을 포함한다. 단순 BOM 변경·라벨 변경 등 시스템 아키텍처에 영향이 없는 경미 변경은 본 지침을 적용하지 않고 [[PRO-SPICE-01-08_문제및변경관리프로세스]] 의 SUP.10 만 적용한다.
...
- **Output**
  - 이해관계자 식별 대장 (TMP-SPICE-01-01-01-01 §1 부분)
  - Stakeholder Requirements Specification (StRS) v1.0 (TMP-SPICE-01-01-01-01 §2~5)
  - 의사소통 채널 합의서 (회의 주기·보고 채널)
  - StRS 베이스라인 등록 기록 ([[PRO-SPICE-01-07_품질보증및형상관리프로세스]] CM 등록)
...
1. **이해관계자 식별** (SYS.1.BP1)
   ...
   - 결과: TMP-SPICE-01-01-01-01 §1 "이해관계자 대장" 작성.
...
3. **요구사항 분류·문서화** (SYS.1.BP3)
   ...
   - 결과: TMP-SPICE-01-01-01-01 §2~4 작성.
...
5. **변경 합의 및 베이스라인** (SYS.1.BP5)
   ...
   - 회신된 코멘트를 [[TMP-SPICE-01-01-01-01]] §5 "변경 이력" 에 기록·반영한다.
...
- OEM 사양서의 표현이 모호하거나(예: "충분히 안전한") 수치가 누락된 경우:
  ...
  - **수행**: TMP-SPICE-01-01-01-01 §6 "Open Issue" 에 등록 → OEM 회신 받을 때까지 해당 요구사항을 `status: pending` 으로 표시 → 회신 도착 시 정식 요구사항화.
...
- 차량 출시 임박 단계에서 OEM 가 긴급 요구사항 변경을 요청한 경우:
  - [[PRO-SPICE-01-08_문제및변경관리프로세스]] 의 SUP.10 변경 절차로 즉시 이관.
...
- 템플릿: [[TMP-SPICE-01-01-01-01_이해관계자요구사항명세서]]
- 작성예시: [[EX-SPICE-01-01-01-01_이해관계자요구사항명세서_작성예시]]
