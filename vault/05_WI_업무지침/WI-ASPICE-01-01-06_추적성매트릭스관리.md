...
# 추적성 매트릭스 관리 업무지침 (WI-ASPICE-01-01-06)

> 상위 절차: [[PRO-ASPICE-01-01_시스템공학프로세스]] §5 단계 7·12 + §7 KPI
...
WI-ASPICE-01-01-01 의 StRS 베이스라인 등록 후부터 SYS.5 검증 보고서 등록 시점까지의 모든 단계에 횡단 적용한다. 다른 시스템공학 WI 와 병렬로 진행되며, 각 베이스라인 등록 시마다 매트릭스 갱신·검증을 수행한다.
...
1. **StRS↔SyRS link** (SYS.2.BP5)
   - WI-ASPICE-01-01-02 완료 시 각 SyRS 의 `derives_from: [STK-...]` 등록.
   - StRS 중 SyRS 로 분해되지 않은 항목 = "고아 StRS" 로 표시.
   - 결과: TMP-ASPICE-01-01-06-01 §1.

2. **SyRS↔Architecture link** (SYS.3.BP5)
   - WI-ASPICE-01-01-03 완료 시 각 아키텍처 요소의 `satisfies: [SYS-...]` 등록.
   - SyRS 중 어느 아키텍처에도 매핑되지 않은 항목 = "고아 SyRS" 표시.
   - 결과: TMP-ASPICE-01-01-06-01 §2.

3. **SyRS↔Verification link** (SYS.5.BP4)
   - WI-ASPICE-01-01-05 완료 시 각 검증 케이스의 `verifies: [SYS-...]` 등록.
   - 미검증 SyRS 식별.
   - 결과: TMP-ASPICE-01-01-06-01 §3.
...
5. **GAP 분석** (정기)
   ...
   - 결과: TMP-ASPICE-01-01-06-01 §4 "GAP 보고서".
...
- 템플릿: [[TMP-ASPICE-01-01-06-01_시스템추적성매트릭스]]
- 작성예시: [[EX-ASPICE-01-01-06-01_시스템추적성매트릭스_작성예시]]
