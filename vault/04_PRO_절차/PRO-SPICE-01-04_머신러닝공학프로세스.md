...
child_wi:
  - "[[WI-SPICE-01-04-01_ML요구사항분석]]"
  - "[[WI-SPICE-01-04-02_ML아키텍처설계]]"
  - "[[WI-SPICE-01-04-03_모델학습]]"
  - "[[WI-SPICE-01-04-04_모델테스트]]"
...
wi_sequence:
  - wi_id: "WI-SPICE-01-04-01"
    mandatory: true
    entry_condition: null
  - wi_id: "WI-SPICE-01-04-02"
    mandatory: true
    entry_condition: "WI-SPICE-01-04-01.status == done"
  - wi_id: "WI-SPICE-01-04-03"
    mandatory: true
    entry_condition: "WI-SPICE-01-04-02.status == done AND PRO-SPICE-01-10.dataset_baseline == ready"
  - wi_id: "WI-SPICE-01-04-04"
    mandatory: true
    entry_condition: "WI-SPICE-01-04-03.status == done"
status: draft
created: "2026-05-06"
updated: "2026-05-06"
...
ADAS·자율주행 인지 모델의 **ML 요구사항(MLE.1) → ML 아키텍처/파이프라인(MLE.2) → 모델 학습(MLE.3) → 모델 테스트(MLE.4)** 사이클을 통제된 흐름으로 수행한다. ASPICE 4.0 의 신규 그룹이며, ML 데이터 거버넌스([[PRO-SPICE-01-10_ML데이터관리프로세스]] / SUP.11) 와 강하게 결합한다.

## 2. 적용 범위

VWAY Motors 가 자체 학습/배포하는 ADAS 인지 모델(차선·객체·신호 인지), 운전자 모니터링 모델, 음성/제스처 인식 모델에 적용한다. 외부 사전학습 모델(pretrained foundation) 의 fine-tuning 도 포함하되, 처음부터 외부 모델을 그대로 통합하는 경우는 [[PRO-SPICE-01-06_구매및공급망프로세스]] 를 우선 적용한다.
...
| ASPICE 조항 | Req-ID | 반영 |
|---|---|---|
| MLE.1 Purpose / BP1 | SPICE-MLE1-R-001/002 | §5 단계 1~2 |
| MLE.2 Purpose / BP3 | SPICE-MLE2-R-001/002 | §5 단계 3~4 |
| MLE.3 Purpose / BP4 | SPICE-MLE3-R-001/002 | §5 단계 5~7, §7 재현성 |
| MLE.4 Purpose / BP3 | SPICE-MLE4-R-001/002 | §5 단계 8~9, §7 KPI/Bias |
