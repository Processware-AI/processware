...
| ML3 | 정의·일관성 (Defined) | 표준 문서화·교육 | **고** — 본 프로젝트의 `/build-standard` 하네스가 이미 처리 |
...
본 프로젝트의 `build-standard` 하네스 위에 운영 레이어를 적층한다.
...
즉 **`/build-standard` 가 만든 문서가 AI Agent 의 행동 명세서로 직결**된다. 이는 일반 표준 도입과의 본질적 차이이며, 차별화의 핵심이다.
...
|---|---|---|
| ML3 도달 시간 | 12~24 개월 | **수일** (`/build-standard`) |
...
- **B**. `/build-standard CMMI-DEV-ML4` 로 ML4 산출물 먼저 빌드 → 그 위에 Agent 운영
...
`/build-standard` 하네스 (Layer 1) 는 운영 자동화 (Layer 2/3) 의 **기반 인프라** 다. 표준 산출물이 Agent 의 명세서로 직결되는 구조는, 전통 컨설팅·일반 RPA·범용 LLM 자동화 모두와 명확히 차별화되는 전략적 자산이다.
