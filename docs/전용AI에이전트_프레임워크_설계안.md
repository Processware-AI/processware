...
> 작성일: 2026-04-17 · 최종 갱신: 2026-05-05 · 상태: 하네스 검증 완료 · Python 포팅 설계 진행 중
> 관련: [[표준프로세스_구성원칙]], 현 하네스(`.claude/agents/`, `.claude/commands/`)

> **하네스 현황 (2026-05-05)**: **4차원 PDCA 폐쇄 루프 + 레거시 백필 + 외부 표준 GAP 분석 PoC 완료**. 총 **29 에이전트 / 7 슬래시 커맨드** 운영 중. 주요 구현 기능 —
> - **차원 0 Ingest**: 9 Phase 파이프라인 + flow-proposer (scope_type: project/org/common 도입)
> - **차원 1 Plan**: 계층적 문서 번호 체계(POL→PRO→WI 계보), 표준 무관(standard-agnostic) — 어떤 구조화 문서든 편입 가능, scope_type 전파(business_flow → PRO → WI), MAT-010 프로세스 플로우맵
> - **차원 2 Do**: WI 이중 포맷(MD+steps.yaml), HITL 다단계 승인·타임아웃·에스컬레이션, 자연어 라우팅(MAT-007)
> - **차원 2B Backfill**: 레거시 문서 → REC 백필 (/process-backfill) — DOCX/XLSX/PDF/PPT 지원, sources/legacy/ Obsidian vault(구조화 MD 보존), 4-신호 WI 자동 매칭(confidence 75% 임계값), OCR 처리(ocrmypdf), 멀티소스 자동 그룹핑(동일 폴더+WI → 1 REC), scope_type 정합성 보정(-10 페널티)
> - **차원 3 Check**: REC↔PRO/WI 대조 9 에이전트, NCR + KPI 대시보드, RBAC 6 역할, act queue 자동 발행
> - **차원 3 Audit**: 외부 표준 GAP 분석 (/process-audit) — coverage_matrix, REC-GAP, confirm 후 critical/major GAP → act queue 자동 push(kind: gap_remediation), --export(XLSX Python스크립트/HTML/PDF)
> - **차원 4 Act**: RCA(5-Why/Fishbone) + PCB quorum + 차원 1 재트리거, --batch 다중 큐 처리
> - **폐쇄 루프**: NCR + critical KPI + critical/major GAP 3종 → act queue 자동 인계. PoC 실증(critical 4→1, healthy 3→6)
...
|---|---|---|---|---|---|
| 0 전처리 | — | `/process-ingest` | 1종 (flow-proposer) | inputs/ 요건 패키지 + business_flow.yaml (scope_type 포함) | **구현됨** |
| 1 자산 구축 | Plan | `/process-plan` | 6종 (analyzer·designer·wi-writer·mapper·qa·flow-mapper) | POL/PRO/WI/TMP/EX/MAT/REF + MAT-010 플로우맵 | **구현됨** |
| 2 이행·관리 | Do | `/process-do` | 5종 (router·executor·hitl·rec-writer·escalation) | REC + MAT-005/007 | **구현됨** |
| 2B 레거시 백필 | Do | `/process-backfill` | 2종 (backfill-matcher·rec-writer) | REC (verdict_type: legacy_evidence) + MAT-005 | **구현됨** |
| 3 심사·평가 | Check | `/process-check` | 9종 (planner·collector·checker·reporter·ncr·kpi×2·independence·trigger) | REC-AUDIT/NCR + MAT-008/009 + act queue | **구현됨** |
| 3 GAP 분석 | Check | `/process-audit` | 3종 (gap-analyzer·gap-reporter·gap-exporter) | REC-GAP + MAT-002 + act queue (gap_remediation) + exports/ | **구현됨** |
| 4 제개정 | Act | `/process-act` | 4종 (rca·planner·pcb·coordinator) | As-Is 입력 + MAT-001 §개정이력 + 차원 1 재트리거 | **구현됨** |

> **이 설계안의 범위**: 차원 0~4 전체(+2B 레거시 백필)가 Claude Code 하네스에서 검증 완료됨. 다음 단계는 **하네스 → 독립 실행 Python 프레임워크 포팅**. 전 차원의 구현 패턴이 정립되어 포팅 설계 범위도 함께 갱신됨.
...
| 4 | **오픈소스/상용** | 미결 |
| 5 | **Obsidian 유지 여부** | 미결 — vault 구조(frontmatter YAML·MD·파일 참조)가 DB 스키마로 직접 전환 가능한 구조임이 확인됨. Obsidian은 편집 UI, DB는 운영·쿼리 레이어로 병행 가능성 높음 |

### 1-C. 미결 결정 항목

1. **언어**: Python 전용 vs TypeScript vs 양쪽?
2. **타겟 고객 최소단위**: 1인 컨설턴트 vs 10인 펌 vs 100인 기업
3. **Obsidian 유지 여부**: 계속 핵심 UI로 / 웹 UI 대체 / 둘 다 — vault 구조(frontmatter YAML→DB 컬럼, MD→Content, 파일 참조→FK, MAT→DB View, steps.yaml→워크플로우 태스크, trace.jsonl→Event log)가 외부 DB·시스템으로 직접 전환 가능. Obsidian은 편집 레이어, DB는 운영 레이어로 공존 가능
...
├── agents/
│   ├── dim0_ingest/             ← 차원 0 (1종) — /process-ingest 하네스 포팅
│   │   └── flow_proposer.py     ← business_flow.yaml 생성 (scope_type 포함)
...
│   │   └── qa_reviewer.py
│   ├── dim2_do/                 ← 차원 2 (5종) — /process-do 하네스 포팅
...
│   ├── dim2b_backfill/          ← 차원 2B (2종) — /process-backfill 하네스 포팅
│   │   ├── backfill_matcher.py  ← 4-신호 WI 매칭 + scope 보정 + 자동 그룹핑
│   │   └── rec_writer.py        ← (dim2_do와 공유, backfill mode 분기)
│   ├── dim3_check/              ← 차원 3 (9종) — /process-check 하네스 포팅
│   │   ├── audit_planner.py
...
│   ├── dim3b_audit/             ← 차원 3 GAP 분석 (3종) — /process-audit 하네스 포팅
│   │   ├── gap_analyzer.py      ← coverage_matrix.yaml 생성
│   │   ├── gap_reporter.py      ← REC-GAP + MAT-002 + act trigger 데이터 반환
│   │   └── gap_exporter.py      ← XLSX 스크립트 / HTML / PDF 내보내기
│   └── dim4_act/                ← 차원 4 (4종) — /process-act 하네스 포팅
...
├── api/                         ← FastAPI 엔드포인트
├── cli/                         ← typer CLI (processmine build-process ISO9001)
├── web/                         ← Next.js 대시보드 (선택)
└── audit/                       ← 감사증적 로거 (OTel)
```
...
- CLI 1개: `processmine build-process ISO9001 --vault ./vault`
...
- **표준 무관(standard-agnostic) 설계**: ISO·IEC·CMMI·법규·사내규정·고객 요구사항 등 어떤 구조화 문서든 편입 가능 ← *하네스 검증됨*
...
- **Backfill Agent (차원 2B)**: sources/legacy/ Obsidian vault 구조화 추출, 4-신호 WI 매칭, 멀티소스 자동 그룹핑, scope_type 정합성 보정, OCR 처리 ← *하네스 검증됨*
- **GAP Audit Agent (차원 3B)**: 외부 표준 GAP 분석, act queue 자동 push(gap_remediation), XLSX/HTML/PDF 내보내기 ← *하네스 검증됨*
- **act-trigger from_gap 모드**: critical/major GAP → act queue 자동 인계 (NCR·KPI·GAP 3종 통합) ← *하네스 검증됨*
...
| **폐쇄 루프 act queue 자동 발행** | 차원 3 NCR/KPI → `.claude/queues/process-act/` 자동 push → 차원 4 RCA → PCB → 차원 1 재트리거 — 전체 루프 패턴 *(하네스 PoC 실증됨)* |
| **PCB quorum 다단계** | simple_majority / supermajority / unanimous / chair_veto 4 모드 — Phase 4.5 명세 완료 *(Python 이식 대상)* |
| **LLM API 운영 비용** | 29 에이전트 자동 실행 시 대용량 컨텍스트 → per-run 토큰 비용이 제품 단가 설계에 직결. 표준 분석 1회 예상 토큰량 측정 후 과금 모델 설계 필요 — **검토 필요** |
| **vault → DB 전환** | frontmatter YAML→컬럼, MD→Content, 파일 참조→FK, MAT→View, steps.yaml→워크플로우 태스크, trace.jsonl→Event log. 단 MD 본문의 Obsidian 링크(`[[...]]`) 파싱 로직 필요 |
| **멀티소스 REC 그룹핑** | 동일 폴더+WI 파일을 1 REC로 병합 시 primary/supplementary 필드 병합 전략 — confidence 기반 자동 결정 + HITL 수동 조정 패턴 *(하네스 검증됨)* |
| **scope_type 전파** | business_flow.yaml → PRO → WI → backfill-matcher 까지 scope_type(project/org/common) 상속 및 보정(-10 페널티) *(하네스 검증됨)* |

---

## 7. 결론 및 첫 수

**가능하고 준비됐다.** 하네스에서 차원 0~4 전체(29 에이전트, 7 슬래시 커맨드, 폐쇄 루프 PoC)가 검증 완료됨. 남은 것은 **실행 엔진을 Claude Code CLI 에서 떼어내는 것**. 각 차원의 구현 패턴이 명확히 정립되어 포팅 범위와 비용을 정확히 예측할 수 있는 상태.

**가장 현실적인 첫 수**:
> "Python + Claude Agent SDK 로 현 `.claude/agents/*` 를 포팅 → `processmine build-process ISO9001` CLI 1개로 차원 1 재현 → `LLMProvider` 추상화 레이어 삽입(멀티 프로바이더 기반 확보) → 차원 2~4 에이전트 순차 포팅 → Phase 3 배포 키트"

→ Phase 1 MVP 예상 소요: **4~6주** (차원 0+1 코어 — 표준 무관 ingest + 계층 번호 생성기 + scope_type 전파 포함)
→ Phase 2 포팅 소요: **8~10주** (차원 2+2B+3+3B+4 에이전트 + 폐쇄 루프 Orchestrator + vault→DB 전환 레이어 + Phase 4.5 외부 연동 기반 구현)
...
- ✅ (N) Backfill 파이프라인 — sources/legacy/ 구조화 추출, 4-신호 WI 매칭(confidence 75%), OCR 처리(×0.85 패널티), 멀티소스 자동 그룹핑, scope_type -10 보정
- ✅ (O) GAP Audit 파이프라인 — coverage_matrix, REC-GAP, act queue 자동 push(gap_remediation), XLSX/HTML/PDF 내보내기
- ✅ (P) act-trigger 3종 통합 — from_audit(NCR) / from_kpi / from_gap(GAP) 모드로 폐쇄 루프 완전체
- ✅ (Q) vault 구조 → DB 전환 가능성 확인 — frontmatter→컬럼, MD→Content, steps.yaml→워크플로우 태스크, trace.jsonl→Event log 매핑 검토 완료
