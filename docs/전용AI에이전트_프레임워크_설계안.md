...
|---|---|---|---|---|---|
| 1 자산 구축 | Plan | `/plan-process` | 5종 (analyzer·designer·wi-writer·mapper·qa) | POL/PRO/WI/TMP/EX/MAT/REF | **구현됨** |
| 2 이행·관리 | Do | `/do-process` | 5종 (router·executor·hitl·rec-writer·escalation) | REC + MAT-005/007 | **구현됨** |
| 3 심사·평가 | Check | `/check-process` | 9종 (planner·collector·checker·reporter·ncr·kpi×2·independence·trigger) | REC-AUDIT/NCR + MAT-008/009 + act queue | **구현됨** |
| 4 제개정 | Act | `/act-process` | 4종 (rca·planner·pcb·coordinator) | As-Is 입력 + MAT-001 §개정이력 + 차원 1 재트리거 | **구현됨** |
...
│   │   └── qa_reviewer.py
│   ├── dim2_do/                 ← 차원 2 (5종) — /do-process 하네스 포팅
...
│   ├── dim3_check/              ← 차원 3 (9종) — /check-process 하네스 포팅
│   │   ├── audit_planner.py
...
│   └── dim4_act/                ← 차원 4 (4종) — /act-process 하네스 포팅
...
| **폐쇄 루프 act queue 자동 발행** | 차원 3 NCR/KPI → `.claude/queues/act-process/` 자동 push → 차원 4 RCA → PCB → 차원 1 재트리거 — 전체 루프 패턴 *(하네스 PoC 실증됨)* |
